...
BEGIN
    ...
    (
        descrip_doc              NVARCHAR(200) NULL,
        docid                    VARCHAR(20)   NOT NULL,
        nro_guia                 VARCHAR(50)   NULL,
        nombre_cliente           VARCHAR(200)  NULL,
        nro_cliente              BIGINT        NULL,
        tipo_documento           NVARCHAR(400) NULL,
        nro_documento            VARCHAR(50)   NULL,
        estado_cabecera          VARCHAR(20)   NULL,
        fecha_act                INT           NULL,  -- yyyymmdd (112)
        id_doc_cod_barra         VARCHAR(100)  NULL,
        tipo_tarjeta             NVARCHAR(100) NULL,
        cod_tipo_tarjeta         NVARCHAR(50)  NULL,
        tipo_producto            NVARCHAR(200) NULL,
    ...
    (
        ...
        tipo_documento, nro_documento, estado_cabecera, fecha_act,
    ...
    SELECT
        ...
        LEFT(CAST(doc.DOCTDC AS NVARCHAR(400)), 400) AS tipo_documento,
        doc.DOCTDN                              AS nro_documento,
        doc.DOCES                               AS estado_cabecera,
        doc.docedf                              AS fecha_act,
        doc.doccb                               AS id_doc_cod_barra,
        LEFT(CAST(doc.docerm AS NVARCHAR(100)),100) AS tipo_tarjeta,
        LEFT(CAST(doc.docori AS NVARCHAR(50)),50) AS cod_tipo_tarjeta,
        LEFT(CAST(doc.docpri AS NVARCHAR(200)),200) AS tipo_producto,
    ...
    WHERE doc.docedf >= @AMD_inicio
      AND (
            (tdc.TDCPRI = 'TARJ.C' AND tdc.TDCID IN (1,36,37,70,89,116,132))
         OR (tdc.TDCPRI = '105'    AND tdc.TDCID = 89)
      );
    ...
    SELECT DISTINCT e.NRO_CLIENTE_TITULAR
    FROM #EventosLogisticaNew e
    WHERE e.NRO_CLIENTE_TITULAR IS NOT NULL
      AND NOT EXISTS (SELECT 1 FROM #ClientesLogistica x WHERE x.valor = e.NRO_CLIENTE_TITULAR);
...
INSERT #AxntContratosRaw
    SELECT
        ...
        CAST(c.td_nombre AS NVARCHAR(200)) AS tipo_documento,
    ...
    (
        ...
        tipo_documento NVARCHAR(200), renombrar_archivo CHAR(2),
    ...
    -- Fallback Axentria solo para clientes sin CI en LS_ULTIMO_CI_AXNT_FULL
    ;WITH faltantes AS (
        SELECT valor FROM #ClientesConContrato
        EXCEPT
        SELECT DISTINCT nro_cliente FROM #CIsFinal
    ),
    CIsAX AS (
        SELECT
            ROW_NUMBER() OVER (
                PARTITION BY TRY_CONVERT(BIGINT, a.vcn_valor)
                ORDER BY e.ve_fechaCreacion DESC, e.VE_ID DESC
            ) AS rn,
            CAST(CONVERT(nvarchar(4), e.ve_fechaCreacion, 112) AS INT)  AS anho,
            CAST(CONVERT(nvarchar(6), e.ve_fechaCreacion, 112) AS INT)  AS anho_mes,
            CAST(CONVERT(nvarchar(8), e.ve_fechaCreacion,112)  AS INT)  AS anho_mes_dia,
        ...
        LEFT JOIN ODSP_axnt_version   e ON a.vcn_iddo = e.ve_iddo
        LEFT JOIN ODSP_axnt_AlmacenFS f ON e.ve_idalmacen = f.alfs_id
        WHERE c.td_id = 104 AND a.vcn_idcm = 21
    ...
    INSERT #CIsFinal
    SELECT
        a.anho, a.anho_mes, a.anho_mes_dia,
        a.vcn_iddo, a.VE_ID,
        a.nombre_fisico, a.nombre_logico,
        a.tipo_documento, a.renombrar_archivo,
        a.nro_guia, a.nro_cliente, a.fecha_creacion
    FROM CIsAX a
    WHERE a.rn = 1;
...
SELECT 
    anho, anho_mes, anho_mes_dia, fecha_creacion, vcn_iddo, VE_ID,
    nombre_fisico, nombre_logico, tipo_documento, renombrar_archivo,
    nro_guia, CAST(nro_cliente AS BIGINT)
FROM #AxntContratosFinal
UNION ALL
SELECT 
    anho, anho_mes, anho_mes_dia, fecha_creacion, vcn_iddo, VE_ID,
