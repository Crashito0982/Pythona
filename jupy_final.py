...
# ----------------------------------------------------------------------
# CONFIGURACIÓN DE RUTAS - JUPYTER / LOCAL
# ----------------------------------------------------------------------
# Ruta directa a la carpeta del día (ajustar según necesites)
ROOT = r"/home/leonardd/Plexus/BRITIMP/CONSOLIDADO/2025-11-04"


def ruta_consolidado_fecha(fecha: str) -> str:
    """
    En la versión Jupyter, ignoramos 'fecha' y usamos ROOT directamente.
    """
    return ROOT
...
def buscar_log(fecha: str) -> Optional[str]:
    """
    Intenta localizar el archivo de log BRITIMP_LOG.txt (o BRITIMP_log.txt)
    dentro de la carpeta de consolidados.
    """
...
def enviar_email_error(fecha_hoy: str, detalle: str, ruta_log: Optional[str] = None) -> None:
    """
    Envía un correo de error del proceso BRITIMP.
    Si se pasa ruta_log y existe, se adjunta el BRITIMP_LOG.txt.
    """
...
def enviar_email_exito(fecha_hoy: str, ruta_log: Optional[str] = None) -> None:
    """
    Envía un correo indicando que el proceso BRITIMP finalizó correctamente.
    Adjunta BRITIMP_LOG.txt si está disponible.
    """
...
def checkea_archivos_creados(fecha: str) -> Tuple[int, list]:
    """
    Verifica que existan todos los archivos consolidados esperados.
...
def run(fecha: Optional[str] = None) -> None:
    """
    Orquesta la subida de tablas para la fecha indicada (AT_CMDTS, Jupyter/local).
    Maneja:
      - Faltan archivos
...
if __name__ == "__main__":
    # En Jupyter / IPython a veces se pasan argumentos (-f), pero aquí los ignoramos
    run()
