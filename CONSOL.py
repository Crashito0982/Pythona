...
# Módulo opcional para notificaciones por correo
try:
    import email  # type: ignore
except ImportError:
    email = None  # type: ignore
    logger.warning("El módulo 'email' no está disponible; no se enviarán notificaciones por correo.
")
...
PENDIENTES = ROOT  # CONTROL-M deja aquí las carpetas ASU, CDE, ENC y OVD con los archivos diarios
...
def enviar_notificacion_exito(fecha_hoy: str) -> None:
    """Envía una notificación (opcional) indicando que el proceso terminó OK."""
    mensaje = (
        "Script para procesamiento de archivos de Bóveda de BRITIMP procesado correctamente.\n\n"
        f"Fecha de proceso: {fecha_hoy}"
    )
    logger.info(f"[NOTIFICACIÓN ÉXITO] {mensaje}")
    if email is None or not hasattr(email, "enviarEmailadjunto"):
        return
    try:
        email.enviarEmailadjunto(
            ...
            asunto=f"BRITIMP - Proceso de consolidado finalizado correctamente ({fecha_hoy})",
            mensaje=mensaje,
            ruta_adjunto=None,
            nombre_adjunto=None,
        )
        logger.info("Notificación de éxito enviada por correo.")
    except Exception as e:
        logger.error(f"No se pudo enviar la notificación de éxito por correo. Detalles: {e}")


def enviar_notificacion_error(fecha_hoy: str, detalle_error: str) -> None:
    """Envía una notificación (opcional) indicando que ocurrió un error."""
    mensaje = (
        "Se produjo un error durante la ejecución del script de procesamiento de archivos de Bóveda de BRITIMP.\n\n"
        f"Fecha de proceso: {fecha_hoy}\n"
        f"Detalle: {detalle_error}"
    )
    logger.error(f"[NOTIFICACIÓN ERROR] {mensaje}")
    if email is None or not hasattr(email, "enviarEmailadjunto"):
        return
    try:
        email.enviarEmailadjunto(
            ...
            asunto=f"BRITIMP - Error en el proceso de consolidado ({fecha_hoy})",
            mensaje=mensaje,
            ruta_adjunto=None,
            nombre_adjunto=None,
        )
        logger.info("Notificación de error enviada por correo.")
    except Exception as e:
        logger.error(f"No se pudo enviar la notificación de error por correo. Detalles: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        enviar_notificacion_error(fecha_hoy, str(e))
        raise
    else:
        enviar_notificacion_exito(fecha_hoy)
