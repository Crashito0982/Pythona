...
def _extraer_saldos_desde_fila(strip_cells: List[str], etiqueta: str = 'SALDO ANTERIOR') -> List[str]:
    """Dada una fila ya strippeada, devuelve los valores no vacíos con dígitos
...
AGENCIA_ALIASES = {
    ...
    'CON': ['CON', 'CNC', 'CONCEPCION', 'CONCEPCIÓN'],
    'ASU': ['ASU', 'ASUNCION', 'ASUNCIÓN'],
    'CDE': ['CDE', 'CIUDAD DEL ESTE', 'C. DEL ESTE', 'C DEL ESTE', 'CDE.'],
}

AGENCIA_FILE_DIGIT_MAP = {'1': 'ASU', '2': 'CDE', '5': 'CON', '3': 'ENC', '4': 'OVD'}
...
def _resolve_agencia(agencia_doc: str, filename: str, filepath: str) -> str:
    ...
    # 2) a partir del nombre de archivo (texto libre + códigos 1_10..5_10)
    code = _agencia_code_from_text(filename)
    if code:
        return code

    m = AGENCIA_FILE_PATTERN.search(filename)
    if m:
        digit = m.group(2)
        code = AGENCIA_FILE_DIGIT_MAP.get(digit)
        if code:
            return code

    # 3) a partir de la ruta de carpetas
    try:
        p = Path(filepath)
...
def get_ec_atm(fecha_ejecucion: datetime, filename: str, dir_entrada: str, dir_consolidado: Optional[str] = None,
    ...
    for _, row in df_raw.iterrows():
        ...
        # Captura de SALDO ANTERIOR (USD, PYG) una sola vez por archivo
    ...
    df_out = pd.DataFrame(registros) if registros else pd.DataFrame(
        columns=['FECHA_OPER','SUCURSAL','RECIBO','BULTOS','GUARANIES','DOLARES','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA']
    )
    # Añadir columnas de SALDO ANTERIOR (se repiten por archivo)
    if not df_out.empty:
        df_out['SALDO_ANTERIOR_PYG'] = saldo_ant_pyg
        df_out['SALDO_ANTERIOR_USD'] = saldo_ant_usd
    ...
    for col in ['FECHA','SUCURSAL','RECIBO','BULTOS','GUARANIES','DOLARES','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA','SALDO_ANTERIOR_PYG','SALDO_ANTERIOR_USD']:
        if col not in df_out.columns: df_out[col] = ''
    df_out = df_out[['FECHA','SUCURSAL','RECIBO','BULTOS','GUARANIES','DOLARES','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA','SALDO_ANTERIOR_PYG','SALDO_ANTERIOR_USD']]
...
def get_ec_banco(fecha_ejecucion: datetime, filename: str, dir_entrada: str, dir_consolidado: Optional[str] = None,
    ...
    for nombre_hoja, df in hojas.items():
        ...
        for _, row in df.iterrows():
            ...
            # Captura de SALDO ANTERIOR por hoja/moneda
            ...
            if ing_egr and motivo_actual and m_date:
                ...
                registros.append({'HOJA_ORIGEN': nombre_hoja,'AGENCIA': (agencia or agencia_fallback),'FECHA_ARCHIVO': fecha_archivo,
                                  ...
                                  'FECHA_OPER': fecha_oper,'SUCURSAL': sucursal,'RECIBO': recibo,'BULTOS': bultos,'MONEDA': moneda_actual,'MONTO': importe,'SALDO_ANTERIOR': saldo_anterior_hoja})
    df_out = pd.DataFrame(registros, columns=['HOJA_ORIGEN','AGENCIA','FECHA_ARCHIVO','ING_EGR','CLASIFICACION','MOTIVO MOVIMIENTO','FECHA_OPER','SUCURSAL','RECIBO','BULTOS','MONEDA','MONTO','SALDO_ANTERIOR'])
    if df_out.empty:
        df_out = pd.DataFrame(columns=['HOJA_ORIGEN','AGENCIA','FECHA_ARCHIVO','ING_EGR','CLASIFICACION','MOTIVO MOVIMIENTO','FECHA_OPER','SUCURSAL','RECIBO','BULTOS','MONEDA','MONTO','SALDO_ANTERIOR'])
    df_out.columns = [str(c).replace(' ', '_').upper() for c in df_out.columns]
...
def get_ec_bultos_atm(fecha_ejecucion: datetime, filename: str, dir_entrada: str, dir_consolidado: Optional[str] = None,
        ...
        if ing_egr and motivo_actual and date_idx is not None:
            ...
            if not (descartar_usd_cero and (str(dolares).replace(',','').replace('.','').strip() in ('','0'))):
                registros.append({'FECHA_OPER': fecha_oper,'SUCURSAL': sucursal,'RECIBO': recibo,'BULTOS': bultos_usd,'MONEDA': 'USD','IMPORTE': dolares,
                                  'ING_EGR': ing_egr,'CLASIFICACION': clasificacion,'FECHA_ARCHIVO': fecha_archivo,'MOTIVO_MOVIMIENTO': motivo_actual,'AGENCIA': (agencia or agencia_fallback)})
    df_out = pd.DataFrame(registros) if registros else pd.DataFrame(
        columns=['FECHA_OPER','SUCURSAL','RECIBO','BULTOS','MONEDA','IMPORTE','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA']
    )
    # Añadir columnas de SALDO ANTERIOR (se repiten por archivo)
    if not df_out.empty:
        df_out['SALDO_ANTERIOR_PYG'] = saldo_ant_pyg
    ...
    for col in ['FECHA','SUCURSAL','RECIBO','BULTOS','MONEDA','IMPORTE','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA','SALDO_ANTERIOR_PYG','SALDO_ANTERIOR_USD']:
        if col not in df_out.columns: df_out[col] = ''
    df_out = df_out[['FECHA','SUCURSAL','RECIBO','BULTOS','MONEDA','IMPORTE','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA','SALDO_ANTERIOR_PYG','SALDO_ANTERIOR_USD']]
    return df_out if collect_only else None
...
def get_ec_bultos_bco(fecha_ejecucion: datetime, filename: str, dir_entrada: str, dir_consolidado: Optional[str] = None,
    ...
    for nombre_hoja, df in hojas.items():
        ...
        for _, row in df.iterrows():
            ...
            if not linea: continue
            linea_up = _strip_accents(linea).upper()
            # Captura de SALDO ANTERIOR (BULTOS_BCO) una sola vez por hoja
            ...
            if ing_egr and motivo_actual and m_date:
                ...
                registros.append({'FECHA': fecha_oper,'SUCURSAL': sucursal,'RECIBO': recibo,'BULTOS': bultos,'MONEDA': moneda_actual or 'PYG','IMPORTE': importe,'SALDO_ANTERIOR': saldo_anterior,
                                  'ING_EGR': ing_egr,'CLASIFICACION': clasificacion,'FECHA_ARCHIVO': fecha_archivo,'MOTIVO_MOVIMIENTO': motivo_actual,'AGENCIA': (agencia or agencia_fallback)})
    df_out = pd.DataFrame(registros, columns=['FECHA','SUCURSAL','RECIBO','BULTOS','MONEDA','IMPORTE','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA','SALDO_ANTERIOR'])
    if df_out.empty: df_out = pd.DataFrame(columns=['FECHA','SUCURSAL','RECIBO','BULTOS','MONEDA','IMPORTE','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA','SALDO_ANTERIOR'])
    df_out.columns = [str(c).replace(' ', '_').upper() for c in df_out.columns]
    for col in ['FECHA','SUCURSAL','RECIBO','BULTOS','MONEDA','IMPORTE','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA','SALDO_ANTERIOR']:
        if col not in df_out.columns: df_out[col] = ''
    df_out = df_out[['FECHA','SUCURSAL','RECIBO','BULTOS','MONEDA','IMPORTE','ING_EGR','CLASIFICACION','FECHA_ARCHIVO','MOTIVO_MOVIMIENTO','AGENCIA','SALDO_ANTERIOR']]
    return df_out if collect_only else None
