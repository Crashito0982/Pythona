...
ROOT = './DATA/BRITIMP' #resolve_root()
ROOT = Path(r"//nfs_airflow_py/cmdat/ea-saa-datos/Transportadoras/Britimp")
PENDIENTES = ROOT  # En esta ruta existen las carpetas ASU, CDE, ENC y OVD con los archivos pendientes
...
def ensure_dirs() -> None:
    """Asegura que todos los directorios necesarios existan en el share de Britimp."""
    # Carpetas base de salida
    for d in [PROCESADO, CONSOLIDADO]:
        d.mkdir(parents=True, exist_ok=True)
    # Carpetas de agencias en PENDIENTES
    for a in AGENCIES:
...
# _LOGGER se mantiene solo por compatibilidad de tipos; el logging real lo hace loguru
...
#    setup_logger()
    ...
    # Volcar los consolidado al AT de datos (at_cmdts)
    try:
