...
BEGIN
    ...
    FROM (
        ...
        where e.ve_fechaCreacion>= @Fecha1
            ...
            AND a.vcn_iddo IN (
                ...
                WHERE v.vcn_idcm = 21 AND TRY_CONVERT(BIGINT, v.vcn_valor) IN (SELECT valor FROM [AT_CMDTS].[dbo].[ClientesParaActivar_PRUEBA])
            )
        UNION
        SELECT
            cast(CONVERT(nvarchar(4), e.ve_fechaCreacion, 112) as int) as anho,
    ...
    FROM (
        ...
        where c.td_id = 104 -- Cédula
            ...
            AND TRY_CONVERT(BIGINT, a.vcn_valor) IN (SELECT valor FROM [ClientesConContrato_PRUEBA] WHERE valor NOT IN (SELECT valor FROM [ClientesEncontradosCI_PRUEBA]))
        UNION ALL 
        SELECT
            ROW_NUMBER() OVER(PARTITION BY TRY_CONVERT(BIGINT, a.vcn_valor) ORDER BY e.ve_fechaCreacion DESC, e.VE_ID DESC) as rn,
            cast(CONVERT(nvarchar(4), e.ve_fechaCreacion, 112) as int) as anho,
