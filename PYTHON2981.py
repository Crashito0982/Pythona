...
BEGIN
    ...
    BEGIN
        ...
        set @sql_2 = 'select
                        ...
                        ,LTRIM(RTRIM(CONVERT(varchar(50), a.DOCCTI))) as nro_cliente
    ...
    SELECT DISTINCT LTRIM(RTRIM(nro_cliente)) AS valor
    INTO [AT_CMDTS].[dbo].[ClientesParaActivar_PRUEBA]
    FROM [ls_eventos_entregados_temp_new_PRUEBA]
    WHERE LTRIM(RTRIM(nro_cliente)) IS NOT NULL AND LTRIM(RTRIM(nro_cliente)) <> ''
    UNION 
    SELECT DISTINCT LTRIM(RTRIM(NRO_CLIENTE_TITULAR)) AS valor
    FROM [ls_eventos_entregados_temp_new_PRUEBA]
    WHERE LTRIM(RTRIM(NRO_CLIENTE_TITULAR)) IS NOT NULL AND LTRIM(RTRIM(NRO_CLIENTE_TITULAR)) <> ''; 
    ...
    FROM (
        ...
        where e.ve_fechaCreacion>= @Fecha1
            ...
            AND a.vcn_iddo IN (
                ...
                WHERE v.vcn_idcm = 21 AND CONVERT(varchar(50), FLOOR(v.vcn_valor)) IN (SELECT valor FROM [AT_CMDTS].[dbo].[ClientesParaActivar_PRUEBA])
            )
        UNION
        SELECT
            cast(CONVERT(nvarchar(4), e.ve_fechaCreacion, 112) as int) as anho,
    ...
    SELECT DISTINCT LTRIM(RTRIM(CONVERT(varchar(50), nro_cliente))) AS valor
    ...
    WHERE LTRIM(RTRIM(valor)) IS NOT NULL AND LTRIM(RTRIM(valor)) <> ''
      AND LTRIM(RTRIM(valor)) IN (SELECT valor FROM [ClientesConContrato_PRUEBA]);
    ...
    FROM (
        SELECT
            ...
            NULL AS nro_guia, LTRIM(RTRIM(b.valor)) as nro_cliente, b.fecha_creacion 
    ...
    FROM (
        SELECT
            ...
            NULL as nro_guia, CONVERT(varchar(50), FLOOR(a.vcn_valor)) as nro_cliente, e.ve_fechaCreacion as fecha_creacion
        from [ODSP_AXNT_VALORCAMPONUM] a
            left join [ODSP_axnt_tipodoc] c on a.vcn_idtd = c.td_id
            left join [ODSP_axnt_contenido] d on a.vcn_iddo = d.co_id
            left join [ODSP_axnt_version] e on a.vcn_iddo = e.ve_iddo
        ...
        where c.td_id = 104 -- Cédula
            ...
            AND CONVERT(varchar(50), FLOOR(a.vcn_valor)) IN (SELECT valor FROM [ClientesConContrato_PRUEBA] WHERE valor NOT IN (SELECT valor FROM [ClientesEncontradosCI_PRUEBA]))
        UNION ALL 
        SELECT
            ROW_NUMBER() OVER(PARTITION BY floor(a.vcn_valor) ORDER BY e.ve_fechaCreacion DESC, e.VE_ID DESC) as rn,
            cast(CONVERT(nvarchar(4), e.ve_fechaCreacion, 112) as int) as anho,
    ...
    FROM (
        SELECT 
            ...
            LTRIM(RTRIM(CONVERT(varchar(50), nro_cliente))) AS nro_cliente
        FROM [ag_axnt_contratos_final_PRUEBA]
        UNION ALL
        SELECT 
            anho, anho_mes, anho_mes_dia, fecha_creacion, vcn_iddo, VE_ID,
            nombre_fisico, nombre_logico, tipo_documento, renombrar_archivo, nro_guia,
            LTRIM(RTRIM(nro_cliente)) AS nro_cliente
