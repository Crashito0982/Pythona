...
BEGIN
    ...
    FROM (
        SELECT
            ...
            CAST(FLOOR(a.vcn_valor) AS BIGINT) as valor,
        ...
        where e.ve_fechaCreacion>= @Fecha1
            ...
            AND a.vcn_iddo IN (
                ...
                WHERE v.vcn_idcm = 21 AND CAST(FLOOR(v.vcn_valor) AS BIGINT) IN (SELECT valor FROM [AT_CMDTS].[dbo].[ClientesParaActivar_PRUEBA])
            )
        UNION
        SELECT
            cast(CONVERT(nvarchar(4), e.ve_fechaCreacion, 112) as int) as anho,
    ...
    FROM (
        SELECT
            ...
            NULL as nro_guia, CAST(FLOOR(a.vcn_valor) AS BIGINT) as nro_cliente, e.ve_fechaCreacion as fecha_creacion
        from [ODSP_AXNT_VALORCAMPONUM] a
            left join [ODSP_axnt_tipodoc] c on a.vcn_idtd = c.td_id
            left join [ODSP_axnt_contenido] d on a.vcn_iddo = d.co_id
            left join [ODSP_axnt_version] e on a.vcn_iddo = e.ve_iddo
        ...
        where c.td_id = 104 -- Cédula
            ...
            AND CAST(FLOOR(a.vcn_valor) AS BIGINT) IN (SELECT valor FROM [ClientesConContrato_PRUEBA] WHERE valor NOT IN (SELECT valor FROM [ClientesEncontradosCI_PRUEBA]))
        UNION ALL 
        SELECT
            ROW_NUMBER() OVER(PARTITION BY floor(a.vcn_valor) ORDER BY e.ve_fechaCreacion DESC, e.VE_ID DESC) as rn,
            cast(CONVERT(nvarchar(4), e.ve_fechaCreacion, 112) as int) as anho,
