...
BEGIN
    ...
    WHERE valor IN (SELECT valor FROM [AT_CMDTS].[dbo].[ClientesParaActivar_PRUEBA]);
    ...
    FROM (
        ...
        where c.td_id = 104 -- Cédula
            ...
            AND floor(a.vcn_valor) IN (SELECT valor FROM [AT_CMDTS].[dbo].[ClientesParaActivar_PRUEBA] WHERE valor NOT IN (SELECT valor FROM [ClientesEncontradosCI_PRUEBA]))
        UNION ALL 
        SELECT
            ROW_NUMBER() OVER(PARTITION BY floor(a.vcn_valor) ORDER BY e.ve_fechaCreacion DESC, e.VE_ID DESC) as rn,
            cast(CONVERT(nvarchar(4), e.ve_fechaCreacion, 112) as int) as anho,
    ...
    INNER JOIN [ls_eventos_entregados_temp_new_PRUEBA] b 
        ON (b.nro_cliente = uc.nro_cliente OR b.NRO_CLIENTE_TITULAR = uc.nro_cliente);
