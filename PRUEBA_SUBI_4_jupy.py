...
# ----------------------------------------------------------------------
# CONFIGURACIÓN DE RUTAS (PRUEBA LOCAL / JUPYTER)
# ----------------------------------------------------------------------

# Para pruebas en Jupyter: ruta directa a la carpeta del día
# Ejemplo:
#   /home/leonardd/Plexus/BRITIMP/CONSOLIDADO/2025-11-04
ROOT = r"/home/leonardd/Plexus/BRITIMP/CONSOLIDADO/2025-11-04"

# En esta versión de prueba, CONSOLIDADO_BASE es la carpeta del día
CONSOLIDADO_BASE = ROOT


def ruta_consolidado_fecha(fecha: str) -> str:
    """
    Versión de prueba local: la carpeta de consolidados YA incluye la fecha,
    por lo que simplemente devolvemos ROOT y se ignora el parámetro fecha.
    """
...
def checkea_archivos_creados(fecha: str) -> int:
    ...
    en la carpeta CONSOLIDADO/{fecha} (en esta versión, simplemente ROOT).
    ...
    if not os.path.isdir(carpeta):
        print("NO EXISTE LA CARPETA DE CONSOLIDADOS PARA ESA FECHA (ROOT)")
        return -1
    ...
    if ok == len(esperados):
        print("SE TIENEN TODOS LOS ARCHIVOS CREADOS EN LA CARPETA CONSOLIDADO (ROOT)")
        return ok
    else:
        print(
...
def _leer_csv_consolidado(fecha: str, nombre_archivo: str) -> Optional[pd.DataFrame]:
    ...
    En esta versión de prueba, la carpeta es siempre ROOT.
    """
    carpeta = ruta_consolidado_fecha(fecha)
...
def run(fecha: Optional[str] = None) -> None:
    ...
    En esta versión de prueba, la fecha solo se usa para logs;
    la ruta de archivos es siempre ROOT.
    """
    if fecha is None:
        fecha = datetime.now().strftime("%Y-%m-%d")

    print(f"FECHA (solo log): {fecha}")  # YYYY-MM-DD
    ...
    if cant_archivos == 5:
        ...
        if continuar:
            print("FINALIZÓ PROCESO DE SUBIDA A TABLAS BRITIMP (PRUEBA LOCAL) CON ÉXITO")
        else:
            print("HUBO ERRORES EN ALGUNA DE LAS SUBIDAS DE TABLAS (PRUEBA LOCAL).")
    else:
        print(
            "ARCHIVOS NECESARIOS PARA POBLAR LAS TABLAS NO FUERON CREADOS. "
            "REVISAR ARCHIVOS EN LA CARPETA ROOT DE PRUEBA."
        )
